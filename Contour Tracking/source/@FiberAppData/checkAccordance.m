...
if isempty(ind) % Image is new
    ...
    this.fibLine = line(this.curIm.xy{1,1}(1,:),this.curIm.xy{1,1}(2,:));      %(cellfun(@(xy) line(xy(1,:), xy(2,:)), this.curIm.xy,'UniformOutput',false));
    ...
    %this.renderFibers(); %Removed this line so that the gui does not
    %continually update, saving time
end

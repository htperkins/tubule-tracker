function OpenImageLoop(hObject, eventdata,file,counter)
FA = guidata(hObject);
...
switch utility.getFileExtension(fileName)
    case {'tif', 'tiff'} % *.tif or *.tiff
        ...
        im = int16(imread(fileName,counter));                  %int16(imread(fullfile(filePath, fileName)));
...
FA.name = [file '(' num2str(counter) ').tif' ];      %fileName;
FA.path = filePath;
FA.im = im;
FA.sizeX = sizeX;
FA.sizeY = sizeY;
